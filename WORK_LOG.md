...
- 2026-08-07: 根据软著申请表和当前软件实际功能，新增说明书用标准流程图
  `docs/manual_assets/NewLight_Analysis_standard_workflow.png`（2400 x 1500，
  200 DPI）。图中覆盖数据导入、实验协议、预览、预处理、ROI 获取与质控、
  dF/F、事件/群体分析及结果导出，并明确标识工作流 JSON 和可选 CUDA/DeepCAD-RT
  分支。图像经 PNG 视觉检查，无节点截断或主流程断线；生成脚本为
  `tools/generate_workflow_chart.py`。
- 2026-08-09: 按说明书排版需求将标准流程图重制为白色背景的中心辐射式
  思维导图。六个编号分支分别对应数据导入、协议与预览、预处理、ROI 获取与
  质控、信号与事件分析、导出与复现；子分支连线均从节点边缘连接，避免穿过
  文本。原文件路径保持不变。
- 2026-08-09: 再次优化标准流程图，解决中心辐射版底部子节点和连线遮挡。
  新版采用六张独立信息卡围绕中央工作流核心，子功能收纳在卡片内部，不再
...
- 2026-09-18: 修正神经光学成像中的背景扣除逻辑。旧实现对每帧做高斯自减并
  将结果整体平移到黑色背景，容易把真实低频荧光和弱信号一起抹掉。新实现使用
  时间维度低分位估计稳定荧光背景，再用空间高斯平滑得到弥散背景，并通过背景
  分位数、空间 sigma 和扣除强度控制；输出仅截断负值，不再强制全局黑底。兼容
  旧工作流的 `sigma` 参数。同步更新中文参数说明并加入背景抑制测试。
- 2026-09-18: 调整 dF/F 自适应带通范围。FFT 检测到原始信号频带后，实际自适应
  下限改为检测下限除以 4，上限改为检测上限乘以 16，再按采样率 Nyquist 频率
  裁剪；界面显示与实际滤波使用同一个 `adaptive_trace_band` 结果，避免状态栏
  与真实参数不一致。完整测试通过 `316 passed, 15 subtests passed`。
